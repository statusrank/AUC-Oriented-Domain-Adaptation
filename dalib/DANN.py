...
class DomainAdversarialLoss(nn.Module):
    ...
    def __init__(self, domain_discriminator: nn.Module, reduction: Optional[str] = 'mean',
                 grl: Optional = None, num_classes = None, multi_label=False, features_dim: Optional[int] = -1, **kwargs):
        super(DomainAdversarialLoss, self).__init__()
        self.grl = WarmStartGradientReverseLayer(alpha=1., lo=0., hi=1., max_iters=500, auto_step=True) if grl is None else grl
        ...
        if not multi_label:
            ...
            self.bce = lambda input, target, weight: \
                F.binary_cross_entropy_with_logits(input, target, weight, reduction=reduction)
        self.domain_discriminator_accuracy = None

    def forward(self, g_s: torch.Tensor, f_s: torch.Tensor, g_t: torch.Tensor, f_t: torch.Tensor,
