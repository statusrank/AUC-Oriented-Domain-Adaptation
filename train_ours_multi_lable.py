...
from models import resnet50, densenet169
...
def main(args):
    ...
    base_path = osp.join(args.root, 
                        ...
                        'class_thres_{}'.format(args.class_thres))
    ...
    for random_seed in args.seed:
        ...
        # backbone = get backbone model (ResNet here)
        logger.info('=====> backbones: {}'.format(args.backbone))

        backbone = backbones[args.backbone](pretrained=True)
        ...
        for epoch in range(args.epochs):
            ...
            logger.info("cur_mean_train_mauc = {:.4f}".format(train_mauc['mean']))
            ...
            best_mauc = max(best_mauc, mauc['mean'])
            ...
            for c in mauc.keys():
                logger.info("Class {}  AUC: {}".format(c, mauc[c]))
...
def validate(val_loader, model):
    ...
    with torch.no_grad():
        for i, (images, target) in enumerate(val_loader):
            ...
            preds.append(F.sigmoid(output).cpu().numpy())
    ...
    mauc = auc_mp_fast(ty, preds, multi_label=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--data', metavar='DATA', default='ChestXray', choices=get_dataset_names(),
                        help='dataset: ' + ' | '.join(get_dataset_names()) +
                             ' (default: Office31)')

    parser.add_argument('-dm', '--da_method', help='method of DA to be run', type=str, default='AUC_DA')
    parser.add_argument('-s', '--source', help='source domain(s)', default=['ChestX-ray14_train'], nargs='+')
    parser.add_argument('-t', '--target', help='target domain(s)', default=['CheXpert_train'], nargs='+')
    ...
    parser.add_argument('--epsilon', type=float, default=5e-3, help="AUC epsilon")
    parser.add_argument('--beta1', type=float, default=1e-2, help="target discrepency beta")
    parser.add_argument('--beta2', type=float, default=1e-2, help="source discrepency beta")
    parser.add_argument('--gamma', type=float, default=1.0, help="weighted function gamma")
    ...
    parser.add_argument('--class_thres', type=float, default=0.5, help="weighted function class_thres")
    parser.add_argument('--decay_epoch', default=1, type=int,
                        help='whether output per-class accuracy during evaluation')
    parser.add_argument('--warm_up_epoch', default=50, type=int,
                        help='whether output per-class accuracy during evaluation')
    
    parser.add_argument('--backbone', choices=['resnet50', 'densenet169'], default='densenet169',
                        help='backbone of model')

    # parser.add_argument('--is-cen', default=False, type=bool, help='use center cropping')
